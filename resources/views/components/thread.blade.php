<article class="p-3 sm:p-5 shadow article-bg relative card-thread sm:border-b-4 sm:border-gray-200 rounded-md " >
    ...
    <div class="relative grid grid-cols-6 gap-2">
        ...
        <span class="text-xs text-gray-500 absolute top-1 left-9 pl-1">{{ $thread->created_at->diffForHumans() }}</span>
            ...
            <a href="{{ route('threads.show', [$thread->category->slug(), $thread->slug()]) }}" class="space-y-2">
                <h2 class="text-sm sm:text-md lg:text-lg sm:font-semibold tracking-wide hover:text-blue-400 text-indigo-400 sm:text-blue-500">
                    {{ $thread->title() }}
                </h2>
                <p class="text-xs sm:text-sm lg:text-md text-gray-700 sm:text-gray-600 thread-p">
                    {{ $thread->excerpt() }}
                </p>
            </a>
            ...
            <div class="flex space-x-3">
                ...
                <div class="flex items-center space-x-2">
                    <x-heroicon-o-heart class="w-5 h-5 sm:w-6 sm:h-6 text-blue-400 border-2 border-gray-200 rounded-lg " />
        ...
        <div class="lex items-center space-x-2">
            <div class="flex space-x-2">
                <div class="hidden md:block p-1 text-xs text-white bg-gray-300 rounded-md">
